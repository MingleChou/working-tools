...
public class ExcOperator {
     ...
     * @return 融合后文件的字节数组
     */
    public static byte[] merge(InputStreamResource[] fileInputAsResource) throws IOException {
        // 拿出多个Excel表格
        ...
        //默认Excel只有1个sheet，也只处理第一个sheet
        XSSFSheet sheet = workbooks[0].getSheetAt(0);
        ...
        for(int i = 1; i < workbooks.length; i++){
            XSSFSheet tmpSheet = workbooks[i].getSheetAt(0);
